...
7. 다른 반이나 다음 차시용 방이 필요하면 `새 수업방 만들기`를 다시 누릅니다. 기존 방은 유지되며 `내 수업방` 목록에서 원하는 방을 선택할 수 있습니다.
8. 학생들이 참여하면 선택한 방의 대시보드가 Firestore `onSnapshot`으로 갱신됩니다.
9. 학생 한 명의 `보기`를 누르면 판단 이유를 라운드별로 확인할 수 있습니다.
10. 비교 체크박스로 2명을 고른 뒤 `선택한 2명 비교`를 누르면 나란히 비교할 수 있습니다.
...
6. `선택하고 다음 시장 보기`를 누르면 판단 입력 영역이 사라지고 같은 위치에 `라운드 결과`가 표시됩니다.
7. `다음 라운드`를 누르면 다시 다음 판단 화면으로 전환됩니다.
8. 20라운드 후 자신의 결과와 단순 전략 A/B/C를 비교합니다.
...
`firebase-config.js`의 아래 값만 수정하면 됩니다.
