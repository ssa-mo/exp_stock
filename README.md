...
1. Pages 주소에 접속합니다.
2. `교사 모드`를 누릅니다.
3. 교사 암호키를 입력합니다. 기본값은 `235math`입니다.
4. `새 수업방 만들기(방 이름 입력)`를 누릅니다.
5. 화면에 표시된 정보를 확인합니다.
   - 방 코드
   - 시장 시드
   - 참여 링크
   - 현재 참여 인원
6. 학생에게 방 코드 또는 참여 링크를 공유합니다.
7. 다른 반이나 다음 차시용 방이 필요하면 `새 수업방 만들기(방 이름 입력)`를 다시 누릅니다. 기존 방은 유지되며 `내 수업방` 목록에서 원하는 방을 선택할 수 있습니다. 목록에는 방 이름·방 코드·라운드 수만 표시되고 시장 시드는 숨겨집니다.
8. 선택한 방의 `방 이름 수정` 버튼으로 기존 방의 이름을 바꿀 수 있습니다. 방 코드와 시장 데이터는 그대로 유지됩니다.
9. 학생들이 참여하면 선택한 방의 대시보드가 Firestore `onSnapshot`으로 갱신됩니다.
10. 학생 한 명의 `보기`를 누르면 판단 이유를 라운드별로 확인할 수 있습니다.
11. 비교 체크박스로 2명을 고른 뒤 `선택한 2명 비교`를 누르면 나란히 비교할 수 있습니다.
...
2. `학생으로 참여`를 누릅니다.
3. 별명과 방 코드를 입력합니다.
4. 매 라운드 먼저 `투자 / 회수 / 유지` 중 하나를 고릅니다.
5. 투자 또는 회수를 고르면 금액 설정 영역이 나타납니다. 정확한 금액을 1원 단위로 직접 입력하거나, 현재 가능한 최대 금액에 맞춰 자동 생성되는 빠른 조정 버튼과 `전액` 버튼을 사용할 수 있습니다. 소수점 이하는 버립니다.
6. `왜 이렇게 선택했나요?`에 판단 이유를 적습니다.
7. `선택하고 다음 시장 보기`를 누르면 판단 입력 영역이 사라지고 같은 위치에 `라운드 결과`가 표시됩니다.
8. `다음 라운드`를 누르면 다시 다음 판단 화면으로 전환됩니다.
9. 20라운드 후 자신의 결과와 단순 전략 A/B/C를 비교합니다.
...
예를 들어 투자 평가금액이 500,000원이고 이번에 137,500원을 투자한 뒤 다음 시장이 -3%라면:

```text
행동 직후 투자금 = 637,500원
시장 변화 = -3%
637,500 × 0.97 = 618,375원
```

현금이 362,500원이라면:

```text
총자산 = 362,500 + 618,375 = 980,875원
```
...
const SETTINGS = Object.freeze({
  ...
  STRATEGY_B_STEP: 100_000,       // 자동 비교 전략 B에서만 사용하는 금액
...
`app.js`의 `SETTINGS.TEACHER_ACCESS_KEY` 값만 수정하면 됩니다.
...
예를 들어 `myclass2026`으로 바꾸려면 문자열만 교체합니다. 이 암호키는 GitHub Pages의 프론트엔드 코드 안에 포함되므로 **강력한 관리자 보안 수단이 아니라 학생의 실수나 일반적인 임의 진입을 막는 수업용 접근 장치**입니다. 실제 데이터 권한은 계속 Firebase Authentication과 Firestore Security Rules가 담당합니다.
...
- 교사 모드 클릭 시 암호키 입력창이 나오고 `235math`로만 진입되는지
- 교사 기기에서 방을 2개 이상 만들었을 때 기존 방이 `내 수업방` 목록에 그대로 남는지
- 방 목록을 눌렀을 때 해당 방 학생 대시보드로 전환되는지
...
`app.js` 상단의 `SETTINGS.TEACHER_ACCESS_KEY` 값을 바꾸면 됩니다. 기본값은 `235math`입니다. GitHub Pages의 프론트엔드에 포함되는 값이므로 강력한 관리자 인증이 아니라 학생의 일반적인 임의 진입을 막는 수업용 잠금장치입니다.
